 ...
 */
    ...
    class io_dzz extends io_api
    {
        ...
        public function getStream($path, $fop = '')
        {
            ...
            if (strpos($path, 'attach::') === 0) {
                ...
                if ($bz == 'dzz') {
                    ...
                    if ($fop && !file_exists($filepath)) return array('error' => 'file is not exists');
                    return $filepath;
                } else {
                ...
                if ($bz == 'dzz') {
                    if ($icoarr['type'] == 'videolink' || $icoarr['type'] == 'dzzdoc' || $icoarr['type'] == 'link') {
                        return $icoarr['url'];
                    }
        ...
        public function getFileUri($path, $fop = '')
        {
            ...
            if (strpos($path, 'attach::') === 0) {
                ...
                $bz = io_remote::getBzByRemoteid($attach['remote']);
                if ($bz == 'dzz') {
                    return  $_G['setting']['attachurl'] . $attach['attachment'];
                } else {
                ...
                if (strpos($path, './') !== false) return '';
                return  $_G['setting']['attachurl'] . preg_replace("/^dzz::/", '', $path);
            } elseif (strpos($path, 'TMP::') === 0) {
                return  'index.php?mod=io&op=getStream&path=' . dzzencode($path);
            } else {
                ...
                if ($icoarr['aid']) {
                    ...
                    if ($bz == 'dzz') {
                        ...
                        return  $_G['setting']['attachurl'] . $icoarr['attachment'];
                    } else {
        ...
        public function deleteThumb($path, $width = 0, $height = 0)
        {
            ...
            foreach ($_G['setting']['thumbsize'] as $value) {
                $target = ($cachepath) . '.' . $value['width'] . '_' . $value['height'] . '_1.jpeg';
                $target1 = ($cachepath) . '.' . $value['width'] . '_' . $value['height'] . '_2.jpeg';
        ...
        public function createThumbByOriginal($path,$width = 0, $height = 0, $thumbtype = 1,$original=0,$extraparams=array(),$filesize=0)
        {
            ...
            if(strpos($path, 'dzz::') === 0){
                $targetpath = str_replace('dzz::','',$path);
            }else{
            ...
            $cachepath = str_replace('//', '/', str_replace(':', '/', $targetpath));
            $extraflag = '';
            ...
            $target = ($cachepath) . '.' . $width . '_' . $height . '_' . $thumbtype . $extraflag . '.'.$filedirextension;
			//兼容老版缩略图
			if(is_file($_G['setting']['attachdir'] . './imgcache/' . $target)){
				return  'dzz::imgcache/' . $target;
			}elseif(is_file($_G['setting']['attachdir'] . './imgcache/ffmpeg/thumb/' . md5('attach::'.$extraparams['aid']).'_1.jpg')){
				return  'dzz::imgcache/ffmpeg/thumb/' . md5('attach::'.$extraparams['aid']).'_1.jpg';
			}
            ...
            if (!in_array($filedirextension, array('png', 'jpg', 'gif', 'jpeg')) || !$imginfo = @getimagesize($fileuri)) {
                ...
                if ($original || ($imginfo[0] < $width || $imginfo[1] < $height)) {
                    if (!$nomark) {
                        if (self::watermark($fileuri, $target, $extraparams)) {
                            $thumbpath = $_G['setting']['attachurl'] . '/' . $target;
                        }
                    }else{
                        $thumbpath = $path;
                    }
                    ...
                    try {
                        ...
                        if ($thumb) {
                            $file = $target_attach;
                            if (!$nomark) self::watermark($file, '', $extraparams);
                            $thumbpath = $_G['setting']['attachurl'] . '/' . $target;
                        } else {
                            $thumbpath = false;
                        }
            ...
            //如果缩略图位置不在本地,移动缩略图到目标位置
            if (strpos($thumbpath, $_G['setting']['attachurl']) === 0) {
                    $thumbpath = str_replace($_G['setting']['attachurl']. '/', 'dzz::', $thumbpath);
                }
            return $thumbpath;
        ...
        //获取缩略图临时文件
        public function getRealThumb($path, $width = 0, $height = 0, $original = false,  $thumbtype = 1, $extraparams = array())
        {
            global $_G;
            ...
            //缩略图记录表
            $thumbrecodearr = [
                'aid' => $data['aid'] ? $data['aid'] : str_replace("attach::", '', $data['icoid']),
                'width' => $width,
                'height' => $height,
            ...
            //已存在缩略图不需要再生成
            if (isset($thumbrecorddata['thumbstatus']) && $thumbrecorddata['thumbstatus']) {
                    $thumbpath = $thumbrecorddata['path'];
            }else{
                $bz = io_remote::getBzByRemoteid($data['remote']);
                $attachmentpath = $data['attachment'] ? $data['attachment'] : $data['path'];
                $fileurl = ($bz == 'dzz') ? 'dzz::'.$attachmentpath:$bz.'/'.$attachmentpath;
                $thumbpath =  IO::createThumbByOriginal($fileurl,$width,$height,$thumbtype,$original,$extraparams);
                if(!$thumbpath){
                    //如果不符合云端生成缩略图条件,调用服务器挂载点缩略图生成
                    $fileurls = array();
                    Hook::listen('thumbnail', $fileurls, $path);//调用挂载点程序生成缩略图绝对和相对地址；
                    if ($fileurls) {
                        $thumbrecorddata = C::t('thumb_record')->fetch_original($data['aid']);
                        $fileurl = $thumbrecorddata['path'];
                    }
            ...
            if(!$thumbpath || strpos($thumbpath,'dzz/') === 0) return false;
            $img = IO::getStream($thumbpath);
            return $img;
        }
        
        //获取缩略图
        public function getThumb($path, $width = 0, $height = 0, $original = false, $returnurl = false, $create = 0, $thumbtype = 1, $extraparams = array(),$filesize=0)
        {
            global $_G;
            
            //$path:可能的值 icoid,'dzz::dzz/201401/02/wrwsdfsdfasdsf.txt'等dzzPath格式；
            if (!$data = IO::getMeta($path)) return false;
			$extraparams['aid']=$data['aid'];
            //如果是目录直接返回后缀图
            if ($data['type'] == 'folder') {
                $img = isset($data['img']) ? $data['img'] : geticonfromext($data['ext'], $data['type']);
            ...
            if (!$create) {
                ...
                //如果缩略图生成次数大于10次，直接返回文件后缀图，不再生成缩略图
                if (isset($thumbrecorddata['thumbstatus']) && !$thumbrecorddata['thumbstatus'] && isset($thumbrecorddata['times']) && $thumbrecorddata['times'] > 10) {
                    $img = geticonfromext($data['ext'], $data['type']);
                    if ($create) C::t('thumb_record')->update($thumbrecorddata['id'], array('thumbstatus' => 1, 'dateline' => time(), 'path' => $img));
                    if ($returnurl) return $img;
                    else  IO::output_thumb($img);
                }
                else {
                   $bz = io_remote::getBzByRemoteid($data['remote']);
                    $attachmentpath = $data['attachment'] ? $data['attachment'] : $data['path'];
                    if(  $thumboriginalrecorddata = C::t('thumb_record')->fetch_original($thumbrecodearr['aid'])){
                        $fileurl = $thumboriginalrecorddata['path'];
                        $filesize = $thumboriginalrecorddata['filesize'];
                    }else{
                        $fileurl = ($bz == 'dzz') ? 'dzz::'.$attachmentpath:$bz.'/'.$attachmentpath;
                        $filesize = $data['filesize'] ? $data['filesize'] : $data['size'];
                    }
                    $thumbpath =  IO::createThumbByOriginal($fileurl,$width,$height,$thumbtype,$original,$extraparams,$filesize);
                    //如果生成失败createThumbByOriginal
                    if(!$thumbpath){
                        //如果不符合云端生成缩略图条件,调用服务器挂载点生成原图后再次尝试生成
                        $fileurls = array();
                        Hook::listen('thumbnail', $fileurls, $path);//调用挂载点程序生成缩略图绝对和相对地址；
                        if ($fileurls) {
                            $thumboriginalrecorddata = C::t('thumb_record')->fetch_original($thumbrecodearr['aid']);
                            $fileurl = $thumboriginalrecorddata['path'];
                            $filesize = $thumboriginalrecorddata['filesize'];
                            //生成缩略图
                            $thumbpath =  IO::createThumbByOriginal($fileurl,$width,$height,$thumbtype,$original,$extraparams,$filesize);
                    ...
                    if(!$thumbpath) {
                        $thumbpath = isset($data['img']) ? $data['img'] : geticonfromext($data['ext'], $data['type']);
                        C::t('thumb_record')->update($thumbrecorddata['id'], array('thumbstatus' => 1, 'dateline' => time(), 'path' => $thumbpath, 'remoteid' => 0));
                        $img = IO::getFileUri($thumbpath);
                    }else{
                        C::t('thumb_record')->update($thumbrecorddata['id'], array('thumbstatus' => 1, 'dateline' => time(), 'path' => $thumbpath, 'remoteid' => $data['remote']));
                        $img = IO::getFileUri($thumbpath);
                    }
                    if ($returnurl) return $img;
                    else IO::output_thumb($img);
    ...
    private function coverFilebynewVersion($attach, $icoarr)
    {
        ...
        public function setFileContent($rid, $fileContent, $force = false, $nocover = true)
        {
            ...
            if ($covertype) {
                if ($covertype == 2) {
                    ...
                    $vinfo = DB::fetch_first("select vid from %t where rid = %s order by dateline", array('resources_version', $icoarr['rid']));
        ...
        public function setFileContent2($rid, $fileContent, $force = false, $nocover = true, $datetime, $pointname)
        {
            global $_G;
            if (!$icoarr = C::t('resources')->fetch_by_rid($rid)) {
                return array('error' => lang('file_not_exist'));
            ...
            if ($covertype) {
                ...
                $setarr = array(
                    'uid' => $_G['uid'],
                    'username' => $_G['username'],
                    'vname' => $_G['username'] . '_' . $pointname . '_' . $datetime,
                    'name' => $icoarr['name'],
                    'aid' => $attach['aid'],
                    'size' => $attach['filesize'],
                    'ext' => $attach['filetype'],
                ...
                $return = C::t('resources_version')->add_new_version_by_rid2($icoarr['rid'], $setarr, $force, $datetime, $pointname);
                if ($return['error']) {
                    return array('error' => $return['error']);
                }
            } else {
        ...
        //云端打包下载
        public function zipclouddownload($paths, $filename = '', $checkperm = 'download', $params = array())
        {
            global $_G;
            $paths = (array)$paths;
            set_time_limit(0);
            ...
            $info = C::t('resources')->fetch_info_by_rid($paths[0]);
            $path = C::t('resources_path')->fetch_pathby_pfid($info['pfid']);
            $hash = C::t('resources_event')->get_showtpl_hash_by_gpfid($info['pfid'], $info['gid']);
            ...
            if (count($paths) > 1) {
                ...
                C::t('resources_event')->addevent_by_pfid($info['pfid'], 'downfiles', 'down', $eventdata, $info['gid'], '', $filenames);
            } else {//如果为单个时
                $eventdata['files'] = $info['name'];
                C::t('resources_event')->addevent_by_pfid($info['pfid'], 'downfile', 'down', $eventdata, $info['gid'], $info['rid'], $info['name']);
            }
            
            if (!$params['identify']) {
                $params['identify'] = get_flag_by_pfid($info['pfid']);
            }
            ...
            include_once libfile('function/packzip');
            $zipfilepath = $_G['setting']['attachdir'] . '/cache/tmpdownloadpack/' . $filename . '.zip';
            $drecordid = C::t('downfile_record')->zippackfilepack($zipfilepath, $paths, $eventdata['position'], $checkperm, $params);
            return $drecordid;
            exit();
        }
        
        //打包下载文件
        public function zipdownload($paths, $filename = '', $checkperm = 'download', $params = array())
        {
            global $_G;
            $paths = (array)$paths;
            set_time_limit(0);
            ...
            if (count($paths) > 1) {
                ...
                $filenames = getstr($filename, 0, 60);
            ...
            if (!$params['identify']) {
                $params['identify'] = get_flag_by_pfid($infos['pfid']);
            }
            $filename = (strtolower(CHARSET) == 'utf-8' && (strexists($_SERVER['HTTP_USER_AGENT'], 'MSIE') || strexists($_SERVER['HTTP_USER_AGENT'], 'Edge') || strexists($_SERVER['HTTP_USER_AGENT'], 'rv:11')) ? urlencode($filename) : $filename);
            ...
            $data = self::getFolderInfo($paths, $eventdata['position'], $zip, $checkperm, $params);
            
            $zip->finalize();
        }
        
        public function getFolderInfo($paths, $position = '', &$zip, $checkperm = '', $params = array())
        {
            static $data = array();
            try {
                    ...
                    switch ($meta['type']) {
                        case 'folder':
                            ...
                            foreach ($contents as $key => $value) {
                                self::getFolderInfo(array($value['rid']), $lposition, $zip, $checkperm, $params);
                            }
                            break;
                        case 'discuss':
                        case 'dzzdoc':
        ...
        public function moveFileToDownload($path, $filepath)
        {
            ...
            $stream = IO::getStream($filepath);
            if (file_put_contents($path, file_get_contents($stream)) === false) {
                return array('error' => 'put file failer');
            } else {
                return $path;
            }
        }
        ...
        public function download($paths, $filename = '', $checkperm = 'download', $param = array())
        {
            ...
            if (count($paths) > 1) {
                return self::zipclouddownload($paths, $filename, $checkperm, $param);
                exit();
            } else {
                $path = $paths[0];
            }
            ...
            if (!$defaultdownloadremoteid) {
                $defaultdownloadremoteid = $icoarr['remote'];
            }
            ...
            if ($bz == 'dzz') {
                ...
                //如果需要添加时间记录,添加下载事件记录
                if ($needeventdata && $eventdata) {
                    C::t('resources_event')->addevent_by_pfid($icoarr['pfid'], 'downfile', 'down', $eventdata, $icoarr['gid'], $icoarr['rid'], $icoarr['name']);
                }
                $pathpre = md5($icoarr['pfid']);
                $fbz = io_remote::getBzByRemoteid($icoarr['remote']);
                //下载区目标位置路径
                $cloudpath = $bz . '/tmpdownload/' . $pathpre . '/' . $attachment['name'];
                //原文件位置路径
                $fpath = ($fbz == 'dzz' ? 'dzz::' : $fbz) . '/' . $attachment['attachment'];
                //检查下载记录表是否已存在对应数据，不存在则插入数据，返回记录表id
                $recordid = C::t('downfile_record')->chkdownloadrecord($cloudpath, $fpath, $attachment, $attachurl, $defaultdownloadremoteid);
                if ($icoarr) {
        ...
        public function Delete($path, $finaldelete = false, $force = false)
        {
            ...
            if (strpos($path, 'dzz::') === 0) {
                ...
                //if(strpos($path,'eagle') === 0) return true;
        ...
        public function uploadToattachment($attach, $fid,$nohook=0)
        {
            ...
            global $_G, $documentexts, $space, $docexts,$Types;
            ...
            if ($icoarr['rid']) {
                ...
                if(!$nohook){
                    ...
                    //主动模式生成缩略图
                    if (in_array($icoarr['ext'], $Types['video'])) {
                        $thumbrecodearr = [
                            'aid' => $icoarr['aid'],
                        ...
                        try {
                            ...
                            foreach ($settingthumbsize as $key => $value) {
                                ...
                                if (isset($thumbrecorddata['thumbstatus']) && $thumbrecorddata['thumbstatus'] && $thumbrecorddata['path']) {
                                    ...
                                    if ($key == 'large') $icoarr['url'] = IO::getFileUri($thumbrecorddata['path']);
                                }else{
									 if ($key == 'small') {
										$icoarr['img']= geticonfromext($icoarr['ext'], $icoarr['type']);
									 }else{
										 $icoarr['url']=DZZSCRIPT . '?mod=io&op=thumbnail&size=large&create=1&path=' . dzzencode($icoarr['rid']);
									 }
								}
                            }
                        } catch (Exception $e) {
        ...
        /*秒传时调用*/
        public function secondUpload($filename, $attach, $fid,$overwrite=0,$nohook=0)
        {
            ...
            if($overwrite){
                if($rid = DB::result_first("select rid from %t where name = %s and pfid = %d",array('resources',$filename,$fid))){
                    return C::t('resources')->fetch_by_rid($rid);
                }
            }
            $gid = DB::result_first("select gid from %t  where fid=%d", array('folder', $fid));
            ...
            if (in_array(strtolower($attach['filetype']), $imgexts)) {
                $icoarr = array(
                    'uid' => $_G['uid'],
                    'username' => $_G['username'],
                    'name' => self::getFileName($filename, $fid),
                    'dateline' => $_G['timestamp'],
                    'pfid' => intval($fid),
                    'type' => 'image',
                    'flag' => '',
                ...
                if ($icoarr['rid'] = C::t('resources')->insert_data($icoarr)) {//插入主表
                    ...
                    if (C::t('resources_attr')->insert_attr($icoarr['rid'], $icoarr['vid'], $sourceattrdata)) {//插入属性表
                        C::t('attachment')->update($attach['aid'], array('copys' => $attach['copys'] + 1));//增加图片使用数
                        $icoarr = array_merge($attach, $icoarr, $sourceattrdata);
                        $icoarr['img'] = getThumburl('attach::' . $icoarr['aid'], 'small', 1);
                        $icoarr['url'] = getThumburl('attach::' . $icoarr['aid'], 'large', 1);
                        $icoarr['bz'] = '';
                        $icoarr['rbz'] = io_remote::getBzByRemoteid($attach['remote']);
                        $icoarr['relativepath'] = $path . $icoarr['name'];
                        $icoarr['path'] = $icoarr['rid'];
            ...
            if ($icoarr['rid']) {
                ...
                $icoarr['sperm'] = perm_FileSPerm::typePower($icoarr['type'], $icoarr['ext']);
                if(!$nohook){
                    Hook::listen('createafter_addindex_getvideo', $icoarr);
                    Hook::listen('createafter_addindex', $icoarr);
                }
            ...
            if (!$path) {
                ...
                foreach ($patharr as $fname) {
                    ...
                    if ($fid = DB::result_first("select fid from %t where pfid=%d and isdelete<1 and fname=%s", array('folder', $pfid, $fname))) {
                        ...
                        if ($re = self::CreateFolder($data['pfid'], $fname, 0, $params, 'overwrite',true)) {
							if( $re['icoarr']){
        ...
        public function uploadStream($file, $filename, $pfid, $relativePath = '', $content_range = array(), $params = array())
        {
            ...
            if (substr($filename, -7) == '.folder') {
                ...
                if (empty($data['folderarr'])) {
                    ...
                    $folderdata = C::t('folder')->fetch_by_fid($data['pfid']);
                    if (empty($folderdata) || $folderdata['isdelete']) {
                        return array('error' => 'folder is not exists');
                    }
                    $data['folderarr'][] = $folderdata;
                }
        ...
        public function overwriteUpload($fileContent, $rid, $filename, $partinfo = array())
        {
            ...
            $gid = DB::result_first("select gid from %t where fid=%d", array('folder', $icoarr['pfid']));
            if (in_array($icoarr['type'], array('folder', 'link', 'videolink', 'dzzdoc'))) {
           ...
           // $target = $icoarr['attachment'];
            ...
            if ($oldaid != $attach['aid']) {
                ...
                C::t('resources_attr')->update_by_skey($icoarr['rid'], $icoarr['vid'], array('aid' => $attach['aid']));
				if($icoarr['vid']) C::t('resources_version')->update($icoarr['vid'], array('aid' => $attach['aid']));
                C::t('attachment')->update($attach['aid'], array('copys' => $attach['copys'] + 1));
                C::t('attachment')->delete_by_aid($oldaid);
            }
            ...
            if ($md5 && $attach = C::t('attachment')->fetch_by_md5($md5)) {
                ...
                if ($target = imagetolocal($link, 'dzz')) {
                    ...
                    if ($attach['aid'] = DB::insert('attachment', ($attach), 1)) {
                        C::t('local_storage')->update_usesize_by_remoteid($attach['remote'], $attach['filesize']);
                        dfsockopen($_G['localurl'] . 'misc.php?mod=movetospace&aid=' . $attach['aid'] . '&remoteid=0', 0, '', '', FALSE, '', 0.1);
        ...
        public function RecovercreateFolderByPath($path, $pfid, $params = array())
        {
            ...
            if (!$path) {
                ...
                foreach ($patharr as $fname) {
                  ...
                  left join %t r on f.fid=r.oid and r.type='folder' where f.fid=%d", array('folder', 'resources', $fid))
                    ) {
                        if ($finfo['isdelete'] > 0) {
                            if ($finfo['rid']) {
        ...
        public function Recover($rid, $combine = true, $force = false)
        {
            ...
            if ($recycleinfo = C::t('resources_recyle')->get_data_by_rid($rid)) {
                /*$pathinfo=$recycleinfo['pathinfo'];
			$patharr=explode(':',$pathinfo);
			$p=explode('_',$patharr[1]);
			if($p[0]=='uid'){//是个人网盘时
				$parent=C::t('folder')->fetch_home_by_uid($p[1]);
				$pfid=$parent['fid'];
				$parr=explode('/',$pathinfo[2]);
				array_shift($parr);
				$path=implode('/'.$parr);
			}elseif($p[0]=='gid'){
				if(!$org=C::t('organization')->fetch($p[1])){
					 return array('rid' => $rid, 'error' => lang('organization no exist'));
				}
				$pfid=$org['fid'];
                ...
                if ($dirinfo['path']) {
                    //若目录被删除恢复或创建目录
                    if (!$folderinfo = self::RecovercreateFolderByPath($dirinfo['path'], $dirinfo['pfid'])) {
                        return array('rid' => $rid, 'error' => lang('file_longer_exists'));
                    }
                }
            ...
            if (!$force && !perm_check::checkperm('delete', $icoarr)) {
                ...
                if ($icoarr['type'] == 'folder') {
                    ...
                    if ($combine && ($ricoid = self::getRepeatIDByName($icoarr['name'], $icoarr['pfid'], true))) {
                        ...
                        C::t('folder')->update_recover_by_pfid($rinfo['oid']);
                        /*$fids = C::t('folder')->fetch_sub_by_fid($rinfo['oid']);
                    //目录下所有删除文件rid
                    $rids = array();
					$fids1=array();
                    foreach (DB::fetch_all("select rid,type,oid from %t where pfid in(%n) and isdelete > 0", array('resources', $fids)) as $v) {
                        $rids[] = $v['rid'];
						if($v['type']='folder'){
							$fids1[]=$v['oid'];
						}
					...
					if($rids) {
						C::t('resources')->update_by_rid($rids,array('isdelete' => 0, 'deldateline' => 0));
					}
					if($fids1){
						C::t('folder')->update($fids1,array('isdelete' => 0, 'deldateline' => 0));
					}*/
                        //删除目录
                        if ($ricoid != $icoarr['rid']) {
                            self::delete($icoarr['rid'], true, true);
                        }
                        ...
                        if ($icoarr['isdelete'] > 0) {
                            ...
                            if (C::t('resources')->update_by_rid($rid, $recoverarr)) {
                                ...
                                //修改删除状态
                                //获取当前文件夹fid及所有下级目录fid
                                C::t('folder')->update_recover_by_pfid($icoarr['oid']);
                                /*$fids = C::t('folder')->fetch_sub_by_fid($icoarr['oid']);
                        ...
                        if ($newname) self::rename($icoarr['rid'], $newname);
                        if ($newpfid) {
