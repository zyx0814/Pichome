...
include_once DZZ_ROOT . './core/api/BOS/BaiduBce.phar';
...
class io_BOS extends io_api
{
    ...
    public function authorize($refer = '')
    {
        ...
        if (submitcheck('addspace')) {
            ...
            $bos_config = [
                ...
                'endpoint' => $host,
                'stsEndpoint' =>$extra['stsEndpoint'] ? $extra['stsEndpoint']:''
            ];

            $checkdata = $this->checkBos($bos_config,$bucket);

            if(isset($checkdata['error'])){
                exit(json_encode(array('success'=>false,'msg'=>$checkdata['error'])));
            }
            $cosClient = new Qcloud\Cos\Client($bos_config);
            $type = 'QCOS';
            ...
            $setarr = array(
                ...
                'bucket' => $bucket,
                'dateline' => TIMESTAMP,
                'hostname' => $bos_config['schema'] . ':' . $region,
                'host'=>$host,
    ...
    //验证腾讯云可用性
    public function checkBos($config,$bucket){
        //实例化类
        $bosclient = new BosClient($config);
        $foldername = 'pichomethumb/';
        //创建目录
        try{
            $return =  $bosclient->putObjectFromString($bucket,$foldername,'');
        } catch (Exception $e) {
            return array('error' => $e->getMessage());
        }
        ...
        //创建临时上传目录
        $datafoldername = 'tmppichomeupload/';
        //创建目录
        try{
            $return =  $bosclient->putObjectFromString($bucket,$datafoldername,'');
        } catch (Exception $e) {
            return array('error' => $e->getMessage());
        }
        if(isset($return['error'])){
        ...
        $filecontent = '123';
        try {
            $ret = $bosclient->putObjectFromString($bucket,  $filename, $filecontent);
        } catch (Exception $e) {
            return array('error' => $e->getMessage());
        }
        //获取文件地址
        try {
            $return = $bosclient->getObjectUrl($bucket,$filename,'+120 minutes');;
        }catch (Exception $e) {
            return array('error' => $e->getMessage());
        }
        ...
        //删除文件
        try {
            $response = $bosclient->deleteObject(array('Bucket' =>$bucket, 'Key' =>$filename));
        } catch (Exception $e) {
            return array( 'error' => $e->getMessage());
        }
    ...
    public function createThumbByOriginal($path, $data,$width = 0, $height = 0, $thumbtype = 1, $original = 0, $extraparams = array(),$filesize=0)
    {
        ...
        if(isset($return['error'])){
            ...
            if($data['aid']){
                ...
                $cacheid = C::t('thumb_cache')->insertdata($insertarr);
            }
            if($data['thumbsign'] == 'small'){
                $thumbarr = [
                    'spath'=>$cloudpath,
                    'sstatus'=>1,
                    'scacheid'=>$cacheid,
...
*/
    ...
    public function uploadToattachment($attach, $appid,$fid,$level,$nohook=0)
    {
        ...
        if ($setarr['rid']) {
            ...
            $setarr['realpath'] = IO::getStream('attach::'.$attrdata['path']);
            return $setarr;
        } else {
