...
//获取header字符串
    public function get_http_header_string($headers)
       ...
       public function getAuthorization($secretid, $secretkey, $filename,$method='get')
        ...
        //获取文件地址
        public function getStream($path,$fileexists = false)
        {
            $arr = self::parsePath($path);
            $qcos = self::init($path, 1);
            if (is_array($qcos) && $qcos['error']) return $qcos;
            if ($fileexists) {
                try {
                    $result = $qcos->getObject(array('Bucket' => $arr['bucket'], 'Key' => $arr['object'], 'Range' => 'bytes=0-1'));
                    return $result;
                } catch (Exception $e) {
                    return array('error' => $e->getMessage());
                }
           ...
           /* if($fileexists){
            ...
            if($readperm == 2  ){
                if($this->host){
                    ...
                    $d = (strpos($this->host,'https://') === 0) ? $this->host .'/'. $arr['object']:'https://'.$this->host .'/'. $arr['object'];
                }else{
                    $d = $bucketurl.'/'.$arr['object'];
                }
               // $d = ($this->host) ? 'https://'.$this->host .'/'. $arr['object']:$bucketurl.'/'.$arr['object'];
                ...
                if($this->host) {
                    ...
                    $fileUri = '/'. $arr['object'];
                    $authorization = $this->get_authorization($secret_key,$secret_id,$StartTimestamp, $EndTimestamp, $fileUri, $headers);
                    $d =  (strpos($this->host,'https://') === 0) ? $this->host .'/'. $arr['object'].'?'. $authorization:'https://'.$this->host .'/'. $arr['object'].'?'. $authorization;
