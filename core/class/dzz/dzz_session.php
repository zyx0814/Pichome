...
class dzz_session {
	...
	public $var;
	public $isnew = false;
	private $newguest = array('sid' => 0, 'ip'=>'', 'ip1' => 0, 'ip2' => 0, 'ip3' => 0, 'ip4' => 0,
		...
		if(!$updated) {
			...
			if($_G['uid']) {
				if($_G['cookie']['ulastactivity']) {
					$ulastactivity = intval(authcode($_G['cookie']['ulastactivity'], 'DECODE'));
				} else {
					$ulastactivity = getuserprofile('lastactivity');
					dsetcookie('ulastactivity', authcode($ulastactivity, 'ENCODE'), 31536000);
				}
				//刷新用户登录超时时间
				C::t('admincp_session')->update_by_uid($_G['member']['uid'], $_G['member']['groupid'], array('dateline' => TIMESTAMP, 'ip' => $_G['clientip'], 'errorcount' => -1));
			}
