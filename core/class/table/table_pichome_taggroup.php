    ...
    class table_pichome_taggroup extends dzz_table
    {
        ...
        public function delete_by_cids($cids)
        {
            ...
            if (!empty($cids)) {
                ...
                if(parent::delete($cids)){
                    Hook::listen('lang_parse',$cids,['delTaggroupLangData']);
                    return true;
                }
            }
        ...
        {
            $data =  DB::fetch_all("select * from %t where appid = %s", array($this->_table, $appid));
            Hook::listen('lang_parse',$data,['getTaggroupLangData',1]);
            return $data;
