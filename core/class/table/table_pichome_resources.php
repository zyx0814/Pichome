...
class table_pichome_resources extends dzz_table
{
    ...
    public function update_by_rids($appid, $rids, $attr)
    {
        ...
        foreach($rids as $k=>$rid){
            if(isset($attr['name'])){
                ...
                if($appdata['type'] == 1){
                    if($appdata['charset'] != CHARSET){
                        $attr['name'] = diconv($attr['name'],CHARSET,$appdata['charset']);
                    }

                    //获取文件路径
                    $filepath = DB::result_first("select path from %t where rid = %s",['pichome_resources_attr',$rid]);
                    $newfilepatharr = explode('/',$filepath);
                    $newfilepatharr[count($newfilepatharr)-1] = $attr['name'];
                    $newfile = implode('/',$newfilepatharr);
                    $filerealpath = $appdata['path'] . '/' . $filepath;
                    $newfilepath = $appdata['path'] . '/' . $newfile;
                    if(!rename($filerealpath,$newfilepath)){
                        unset($rids[$k]);
                    }else{
                        C::t('pichome_resources_attr')->update($rid,['path'=>$newfile]);
                    }
                }
            }
        }
        if(!empty($rids)){
            Hook::listen('lang_parse',$attr,['saveResourcesLangData',$rids]);
    ...
    public function delete_by_rid($rids, $uid = 0, $username = '')
    {
        ...
        Hook::listen('lang_parse',$rids,['delResourcesLangData']);
    ...
    public function fetch_by_rids($rids)
    {
        ...
        foreach (DB::fetch_all("select tagname,tid from %t where tid in(%n)", array('pichome_tag', $tagids)) as $tag) {
            Hook::listen('lang_parse',$tag,['getTagLangData']);
        ...
        Hook::listen('lang_parse',$datas,['getResourcesLangData',1]);
        return $datas;
    }
    ...
    {
        $resourcesdata = DB::fetch_first("select r.rid,r.isdelete,r.appid,r.ext,r.type,ra.path as fpath,r.hasthumb,
        ...
        if ($resourcesdata['isdelete']) {
           ...
           if($force && $resourcesdata['apptype'] = 1){
            ...
            $imgdatas = [];
            $iconddatas = $this->getThumbsByrids($rid);
            if($iconddatas[$rid]['icondata'])  $imgdatas['iconimg'] = $iconddatas[$rid]['icondata'];
            if($iconddatas[$rid]['originalimg'])  $imgdatas['originalimg'] = $iconddatas[$rid]['originalimg'];
            if(!$imgdatas['originalimg'] || !$imgdatas['iconimg']){
                $imgdata = $this->getfileimageurl($resourcesdata, $vapppath, $resourcesdata['apptype'], $download);
                if(!$imgdatas[$rid]['iconimg']) $imgdatas['iconimg'] = $imgdata['iconimg'];
                if(!$imgdatas[$rid]['originalimg']) $imgdatas['originalimg'] = $imgdata['originalimg'];
            }
            if ($onlyicon) {

                return $imgdatas['iconimg'];
            } else {
    ...
    public function upload_file($data, $attrover = 1)
    {
        ...
        if ($rid = DB::result_first("select rid from %t where path = %s and appid = %s ", array('pichome_resources_attr', $aid, $appid))) {
            ...
            if (C::t('pichome_resources')->update($rid, $setarr)) {//插入主表
                ...
                //摄影师数据
                if($dataattrs['sys']){
                    $sysarr = !is_array($dataattrs['sys']) ? explode(',', $dataattrs['sys']) : $dataattrs['sys'];
                    foreach($sysarr as $v){
                        $v = getstr($v);
                        if(!$v) continue;
                ...
                foreach ($dataattrs as $k => $val) {
                    if (strpos($k, 'tabgroup_') === 0) {
                        ...
                        foreach($val as $otid){
                            if (!in_array($otid, $odatatids)) {
                                $rtag = ['appid' => $appid, 'rid' => $rid, 'tid' => $otid, 'gid' => $gid];
            ...
            if ($rsetarr['rid'] = C::t('pichome_resources')->insert_data($rsetarr)) {//插入主表
                ...
                if($dataattrs['sys']){
                    ...
                    foreach($sysarr as $v){
                        $v = getstr($v);
                        if(!$v) continue;
                        $rtag = ['appid' => $appid, 'rid' => $rsetarr['rid'], 'labelname' => $v];
                        C::t('pichome_sys')->insert_data($rtag);
                    }
                }
                if ($dataattrs['tag']) {
                    ...
                    foreach ($tagnamearr as $v) {
                        $tid = C::t('pichome_tag')->insert($v, 1);
                        $rtag = ['appid' => $appid, 'rid' => $rsetarr['rid'], 'tid' => $tid];
                        $tids[] = $tid;
                        C::t('pichome_resourcestag')->insert($rtag);
                    }
                    //处理属性表标签数据
                ...
                foreach ($dataattrs as $k => $val) {
                    if (strpos($k, 'tabgroup_') === 0) {
                        $gid = intval(str_replace('tabgroup_', '', $k));
                        foreach($val as $otid){
                            $rtag = ['appid' => $appid, 'rid' => $rsetarr['rid'], 'tid' => $otid, 'gid' => $gid];
                            C::t('pichome_resourcestab')->insert($rtag);
            ...
            if ($setarr['rid'] = C::t('pichome_resources')->insert_data($setarr)) {//插入主表
                ...
                if($dataattrs['sys']){
                    ...
                    foreach($sysarr as $v){
                        $v = getstr($v);
                        if(!$v) continue;
                        $rtag = ['appid' => $appid, 'rid' => $setarr['rid'], 'labelname' => $v];
                        C::t('pichome_sys')->insert_data($rtag);
                    }
                }
                if ($dataattrs['tag']) {
                ...
                foreach ($dataattrs as $k => $val) {
                    ...
                    if (strpos($k, 'tabgroup_') === 0) {
                        $gid = intval(str_replace('tabgroup_', '', $k));
                        foreach($val as $otid){
                            $rtag = ['appid' => $appid, 'rid' => $setarr['rid'], 'tid' => $otid, 'gid' => $gid];
                            C::t('pichome_resourcestab')->insert($rtag);
