...
{
    private $type = array('tabgroup','inputmultiselect','inputselect','input', 'bool', 'time', 'fulltext', 'textarea', 'date', 'timerange', 'select', 'multiselect', 'user', 'label', 'tagcat', 'grade', 'color', 'link');
    ...
    public function fetch($flag, $noisdel = true)
    {
        ...
        if(in_array($data['type'],['inputselect','inputmultiselect'])){
            //获取选项值id
    ...
    public function fetch_by_flag($flag,$opitionlimit=10,$noisdel=true){
        if ($noisdel) {
            $data = DB::fetch_first("select * from %t where flag  =%s and isdel = 0", array($this->_table, $flag));
        } else {
            $data = parent::fetch($flag);
        ...
        if(in_array($data['type'],['inputselect','inputmultiselect'])){
            //获取选项值id
            $data['options'] = C::t('form_filedvals')->get_options_by_filed($flag,$opitionlimit);
        }
        return $data;
    }
    ...
    public function insert_by_flag($flag, $form, $hasnewval = 1)
    {
        ...
        if ((!$setarr['appid'] && !$form['tabgroupid']) && empty($flag)) {
            ...
            if ($form['tabgroupid']) {
                $flag = 'tabgroup_' . $form['gid'] . '_' . time() . random(4);
            }
        }
        ...
        if (isset($form['labelname'])) $setarr['labelname'] = getstr($form['labelname'], 60);
        ...
        if ($hasnewval) {
            if (in_array($form['type'], array('multiselect', 'select'))) {
                ...
                Hook::listen('lang_parse', $form, ['saveFiledoptionsLangData']);
                if ($form['options']) {
                    if ($oflagdata) {
                        $editoptions = [];
                    ...
                    $form['options'] = array_unique($saveoptions);
                }
        ...
        switch ($form['type']) {
            ...
            case 'inputselect':
                //获取所有旧的选项值
                $oldOptionsIds = C::t('form_filedvals')->fetch_ids_by_filed($flag);
                $addOptions = [];
                $editOptions = [];
                $submitIds = [];
                 ...
                 foreach ($form['options'] as $k => $v) {
                        if($v['id']){
                            $submitIds[] = $v['id'];
                            $editOptions[$v['id']] = $v['name'];
                        }else{
                            $addOptions[] = $v['name'];
                        }
                    }
                 //删除的选项
                 $delOptions = array_diff($oldOptionsIds,$submitIds);
                 $delOptions = array_values($delOptions);
                 if($delOptions) C::t('form_filedvals')->delete_by_id($delOptions);
                 //增加的选项
                 $addOptions = array_values($addOptions);
                 $addOptions = array_unique($addOptions);
        ...
        Hook::listen('lang_parse', $form, ['saveFiledLangData']);
        return $setarr;
    }
    ...
    public function fetch_flags_by_appid($appid, $allowsearch = 0, $isnodel = 1)
    {
        ...
        foreach (DB::fetch_all("select * from %t where $sql order by disp", $param) as $value) {
            ...
            if(in_array($value['type'],['inputselect','inputmultiselect'])){
                //获取选项值id
                $value['options'] = C::t('form_filedvals')->get_options_by_filed($value['flag']);
            }
    ...
    public function fetch_all_conditions($flags = array(), $catids = array(), $keyword = '')
    {
        ...
        foreach (DB::fetch_all("select * from %t where $sql order by disp", $param) as $value) {
            ...
            $value['labelname'] = (lang('fs_' . $value['flag']) != 'fs_' . $value['flag']) ? lang('fs_' . $value['flag']) : $value['labelname'];
            //Hook::listen('fileddataFilter',$value);
            $data[] = $value;
        }
        return $data;
    }
    ...
    {
        $flags = [];
        foreach (DB::fetch_all("select flag from %t where filedcat = %d and tabgroupid > 0 ", array($this->_table, $filedcatid)) as $v) {
            $flags[] = $v['flag'];
        }
        return $flags;
    }
