<?php
    ...
    class table_pichome_resources_attr extends dzz_table
    {
        ...
        public function update_by_rids($appid,$rids,$setarr){
            if(!is_array($rids)) $rids = array($rids);
            Hook::listen('lang_parse',$setarr,['saveResourcesLangData',$rids]);
        ...
        public function update_attrs_by_ridFid($rid,$fid,$appid){
            ...
            foreach($attrs as $k=>$v){
                if($k=='tag'){
                    ...
                    foreach($v as $tabid){
                        if(!$tabid) continue;
                        $rtabg = ['appid' => $appid, 'rid' => $rid, 'tid' => $tabid,'gid'=>$gid];
