...
function build_cache_setting() {
    ...
    foreach(DB::fetch_all("select * from %t where `state` = %d ",['language',1]) as $v){
        ...
        $nv= ['langflag'=>$v['langflag'],'isdefault'=>$v['isdefault']];
        if($LanguageList[$nv['langflag']])  $nv = array_merge($nv, $LanguageList[$v['langflag']]);
		if($nv['elementflag']){
			$nv['elementflagCamel'] = 'ElementPlusLocale'.toCamelCase($nv['elementflag'],true);
		}
    ...
    $data['moreLanguageState'] = $moreLangState;
