...
function checkLanguage($langList=array(),$defaultlang='')
{
    global $_G;
    $uid = getglobal('uid');
    ...
    if ($curpath) {
        ...
        if (file_exists(DZZ_ROOT . './' . $curpath . '/language/' . $checkLanguage . '/' . 'lang.php')) {
            ...
            foreach($langlist as $k){
                if(file_exists(DZZ_ROOT . './' . $curpath . '/language/'.$k.'/' . 'lang.php')){
                    include DZZ_ROOT . './' . $curpath . '/language/'.$k.'/' . 'lang.php';
                    break;
                }
            }
...
function curl_file_get_contents($durl, $redirect = 0, $proxy = '')
{
    ...
    if ($redirect) $r = curl_exec_redir($ch);
    else $r = curl_exec($ch);
    curl_close($ch);
    return $r;
}
...
function dzz_app_pic_save($FILE, $dir = 'appimg')
{
    ...
    if (!in_array($ext, $imageext)) return lang('File_format');
    $subdir = $subdir1 = $subdir2 = '';
    $subdir1 = date('Ym');
    $subdir2 = date('d');
    $subdir = $subdir1 . '/' . $subdir2 . '/';
    ...
    if (!$attach = io_dzz::UploadSave($FILE)) {
        return lang('application_failed');
    }
    $setarr = array(
        'uid' => $_G['uid'],
 ...
 * @return bool
 */
function addFileuploadStats($rid, $isadmin = 0)
{
    if (!$rid) return false;
    $filename = DB::result_first("select name from %t where rid = %s", array('pichome_resources', $rid));
    if (!$filename) return false;
    $statsdata = ['statstype' => 1, 'idtype' => 3, 'idval' => $rid, 'name' => $filename, 'isadmin' => $isadmin];
    return addStatsdata($statsdata);
}

function addLoginStats($msg, $isadmin = 0)
{
    $statsdata = ['statstype' => 3, 'msg' => $msg, 'isadmin' => $isadmin];
    return addStatsdata($statsdata);
}
...
//下划线命名到驼峰命名
function toCamelCase($str,$ucfirst=false,$split='-')
{
    $str=strtolower($str);
