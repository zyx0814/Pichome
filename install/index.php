...
elseif($method == 'db_init') {
	...
	if($submit) {
		...
		$uid = 1 ;
		$authkey = 
		md5($_SERVER['SERVER_ADDR'].$_SERVER['HTTP_USER_AGENT'].$dbhost.$dbuser.$dbpw.$dbname.substr(time(), 0, 8)).random(18);
...
elseif($method == 'admin_init') {
	...
	if($submit) {
		...
		if($nickname && $username && $email && $password) {
			if(strlen($nickname) > 30 || preg_match("/^$|^c:\\con\\con$|　|[,\"\s\t\<\>&]|^Guest/is", $nickname)) {
				show_msg('admininfo_nickname_invalid', $nickname, 0);
