...
class UploadHandler
{
    ...
    protected $error_messages = array(
        ...
        'accept_file_names' => 'FileName not formated',
    );
    ...
    protected function validate($uploaded_file,$file, $error,$content_range) {
        ...
        if(preg_match($this->options['notallow_file_type'],$file->name)){
            $file->error = $this->get_error_message('accept_file_types');
            return false;
        }
   ...
   protected function isToCloud(){
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $relativePath, $type, $error,
            $index = null, $content_range = null,$bz='',$container,$pfid) {
        $file = new stdClass();
        ...
        if ($this->validate($uploaded_file, $file, $error,$content_range)) {
            ...
            $filepath = $this->get_upload_path(md5($relativePath.$name).microtime(true));
			...
			try{
				...
				if($return=IO::uploadStream($filepath,$name,$container,$pfid,$relativePath,$content_range)){
    ...
    public function post($print_response = true) { 
		...
		if($content_range && isset($_GET['chunckSize'])){
			$content_range[]=intval($_GET['chunckSize']);
		}
        $files = array();
