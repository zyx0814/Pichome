...
if ($do == 'getHistory') {//获取历史对话
    ...
    if ($type == 'image') {
        ...
        $historydaya = C::t('#ollama#ollama_chat')->fetchContentByIdvalue($idval, $idtype);
        $returndata = [];
    ...
    if ($type == 'image') {
        ...
        C::t('#ollama#ollama_chat')->deleteContentByIdvalue($idval, $idtype);
...
// 从 get 中获取提问
    ...
    if ($type == 'image') {
        ...
        //缺少图片id参数
        if (!$idval) {
            $ollama->end('Missing image id');
        } else {
            //获取对话id
            $metadata = IO::getMeta($idval);
            if (!$metadata)  $ollama->end('file is not exists');
            else {
                $rid = $idval;
                if ($metadata['aid']) {
                    $idval = $metadata['aid'];
                ...
                if(!$imgurl){
                    $ollama->end('file is not allowed');
                }else{
                    ...
                    $return1 = waitLock('DZZ_CHAT_'.$idval,$ollama);
                    $params['processname'] = $return1;


                    $ret=$ollama->getChatData($params);
                    if($ret['error']){
                        dzz_process::unlock($params['processname']);
                        $ollama->end($ret['error']);
                    }
                    @unlink($imgurl);
                }
        ...
        $idval = $_GET['rid'] ?? random(1000,true);
        $idtype = 2;
        $params['idtype'] = $idtype;
        $params['idval'] = $idval;
        $params['question'] = $question;
...
function waitLock($processnameprefix,$ollama){
    $locked = true;
    for($i=0;$i<2;$i++){
