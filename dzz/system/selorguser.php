...
foreach($ids as  $value){
	if(strpos($value,'g_')!==false){
		...
		$orgid=intval(str_replace('g_','',$value));
		if($orgid==0) $orgid='other';
		$orgids[$orgid]=$orgid;
...
if($orgids && $stype!=2){
	if(in_array('other',$orgids)){
		...
		$selects['other']=array('orgid'=>0,'orgname'=>$zero);
	}
	unset($orgids['other']);
	$sel_org=C::t('organization')->fetch_all($orgids);
	foreach($sel_org as $key=> $value){
		$selects[$value['orgid']]=$value;
		$arr=explode('-',str_replace('_','',$value['pathkey']));
		if($arr){
			$count = count($arr);
			if ($open[$arr[$count - 1]]) {
