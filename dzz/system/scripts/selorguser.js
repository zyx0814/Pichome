...
selorg.add=function(ctrlid,vals){
	...
	for(var i in vals){
		...
		var html='';
		if(limit  &&  parseInt(i)>=limit){
			try{window.frames[ctrlid+'_iframe'].selectorg_remove(vals[i].orgid);}catch(e){}
		}else{
			if(vals[i].orgid.indexOf('uid_')===0){
				var headimg = '';
				if(vals[i].icon){
					headimg = '<img src="'+vals[i].icon+'" class="img-circle" title="'+vals[i].text.replace(/<em.+?<\/em>/ig,'')+'">';
				}
