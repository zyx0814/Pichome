		...
		if($_GET['id']=='other'){//无机构用户
			if(($moderator && $_G['adminid']!=1) || $nouser || $stype==1){
				...
				foreach(C::t('organization_user')->fetch_user_not_in_orgid($limit) as $value){
					...
					$uids[] = $value['uid'];
					$tmpdata=array('id'=>'uid_'.$value['uid'],'text'=>$value['username'].'<em class="hide">'.$value['email'].'</em>','icon'=>'dzz/system/images/user.png','state'=>array('disabled'=>$disable),"type"=>$type,'li_attr'=>array('uid'=>$value['uid']));
                    $datas[] = $tmpdata;
				}
			...
			if( $nouser || $stype==1 || ($moderator && !$ismoderator)){
				...
				foreach(C::t('organization_user')->fetch_user_by_orgid($id,$limit) as $value){
                    ...
                    $tmpdata=array('id'=>'orgid_'.$value['orgid'].'_uid_'.$value['uid'],'text'=>$value['username'].($jobname?'<em> ['.$jobname.']</em>':'').'<em class="hide">'.$value['email'].'</em>','icon'=>'dzz/system/images/user.png','state'=>array('disabled'=>$disable),"type"=>$type,'li_attr'=>array('uid'=>$value['uid']));
                    $datas[] = $tmpdata;
				}
				getuserIcon($uids,$datas,$data);
			}
