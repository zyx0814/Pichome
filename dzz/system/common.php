...
if($operation == 'getApp'){
	...
	foreach($app as $key => $value){
		...
		//首页固定排在最前面
		if(getglobal('setting/default_mod')==$value['identifier']){
			$value['disp']=-1000;
		}
	...
	foreach($vapps as $key => $value){
		...
		if(Hook::listen('rolecheck',$params,null,true)===false) continue;
		//首页固定排在最前面
		if(getglobal('setting/default_mod')=='vapp_'.$value['identify']){
			$value['disp']=-1000;
		}
		$applist_1[$value['appid']] = $value;
	}
	...
	exit(json_encode(array('data'=>$applist_1)));
}elseif($operation == 'getNotice'){// 获取通知
	$notice_num=DB::result_first("select COUNT(*) from %t where new>0 and uid=%d",array('notification',$_G['uid']));
	exit(json_encode(array('data'=>$notice_num)));
}else{
	// 地址栏名称
