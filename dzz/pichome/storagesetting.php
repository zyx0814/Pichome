...
if ($do == 'addspace') {
    ...
    foreach($spacelist as $key => $value){
        $spacelist[$key]['cloudname']=lang($value['cloudname']);
    }
    exit(json_encode($spacelist));
} elseif ($do == 'deletespace') {
    $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    if (!$id) exit(json_encode(array('success' => false, 'msg' => lang('params_error'))));
    ...
    if (DB::result_first("select count(appid) from %t where `path` like %s and isdelete < 1", array('pichome_vapp', $bzpath . '%'))) {
        exit(json_encode(array('success' => false, 'msg' => lang('has_use_current_storage_vapp'))));
    } elseif (DB::result_first("select count(aid) from %t where remote = %d", array('attachment', $id))) {
        exit(json_encode(array('success' => false, 'msg' => lang('site_has_file_in_current_storage'))));
    } else {
    ...
    $id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    $connectdata = C::t('connect_storage')->fetch($id);
    $connectdata['cloudname']=lang($connectdata['cloudname']);
    ...
    if ($connectdata['bz'] == 'dzz') {
        ...
        if (C::t("app_market")->update($app['appid'], array("extra" => serialize($appextra)))) {
            ...
            exit(json_encode(array('success' => false, 'msg' => lang('ffmpeg_open_failer'))));
        }
    } elseif ($connectdata['bz'] == 'QCOS') {
        if ($setarr['mediastatus']) {
            ...
            if ($video->check_videobucket()) {
                ...
                exit(json_encode(array('success' => false, 'msg' => lang('please_check_bucket_enable_media'))));
            }
        } else {
            C::t('connect_storage')->update($id, $setarr);
            updateMediaStatus('QCOS:' . $id . ':', $setarr['mediastatus']);
    ...
    if ($connectdata['bz'] == 'dzz') {
        ...
        if ($setarr['docstatus']) {
            ...
            if ($video->check_docbucket()) {
                ...
                exit(json_encode(array('success' => false, 'msg' => lang('please_check_bucket_enable_document'))));
            }
        } else {
