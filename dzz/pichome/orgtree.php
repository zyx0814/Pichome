...
if($_GET['do']=='orgtree'){
	...
	$multiple=intval($_GET['multiple']); //是否允许多选
	$limit=intval($_GET['limit']); //限制选择个数
	$exids = $_GET['exids']?explode(',',$_GET['exids']):array(); //排除的用户
	//判断用户有没有操作权限
	...
	if($_GET['id']=='#'){
		...
		if($stype!=1){
			$data[]=array('id'=>'other','text'=>'<span class="iconFirstWord" style="background:#0099bc;" title="'.$zero.'">无</span>'.$zero,'state'=>array('disabled'=>$disable),"type"=>($type=="disabled")?$type:'default','children'=>true,'label'=>$zero);
		}
		...
		if($_GET['id']=='other'){//无机构用户
			if(($moderator && $_G['adminid']!=1) || $nouser || $stype==1){
				...
				foreach(C::t('organization_user')->fetch_user_not_in_orgid($limit) as $value){
					if($exids && in_array($value['uid'],$exids)) continue;
					$uids[] = $value['uid'];
					$tmpdata=array('id'=>$value['uid'],'text'=>$value['username'].'<em class="hide">'.$value['email'].'</em>','icon'=>'dzz/system/images/user.png','label'=>$value['username'],'state'=>array('disabled'=>$disable),"type"=>$type,'li_attr'=>array('uid'=>$value['uid']));
                    $datas[] = $tmpdata;
				}
			...
			if( $nouser || $stype==1 || ($moderator && !$ismoderator)){
				...
				foreach(C::t('organization_user')->fetch_user_by_orgid($id,$limit) as $value){
                    ...
                    $tmpdata=array('id'=>$value['uid'],'label'=>$value['username'],'text'=>$value['username'].($jobname?'<em> ['.$jobname.']</em>':'').'<em class="hide">'.$value['email'].'</em>','icon'=>'dzz/system/images/user.png','state'=>array('disabled'=>$disable),"type"=>$type,'li_attr'=>array('uid'=>$value['uid']));
                    $datas[] = $tmpdata;
				}
				getuserIcon($uids,$datas,$data);
			}
