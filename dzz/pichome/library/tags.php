...
if ($operation == 'deltag') {//删除标签
    ...
    if (submitcheck('classifysubmit')) {
        ...
        $data['cid'] = $cid;
        $savedata = $data;
        Hook::listen('lang_parse',$savedata,['saveTaggroupLangData']);
        Hook::listen('lang_parse',$data,['getTaggroupLangKey']);
    ...
    $attr = ['cid'=>$cid,'catname'=>$catname];
    Hook::listen('lang_parse',$attr,['saveTaggroupLangData']);
    if($attr)C::t('pichome_taggroup')->update($cid, ['catname' => $catname]);
    exit(json_encode(array('success' => true,'data'=>['cid'=>$cid,'catname'=>$catname])));
}
elseif ($operation == 'gettaggroup') {
    ...
    if (!$cid) {
        if($nocat){
            ...
            Hook::listen('lang_parse',$tagdata,['getTagsLangData']);
        }else{
        ...
        Hook::listen('lang_parse',$tagdata,['getTagLangData',1]);
        Hook::listen('lang_parse',$tagdata,['getTagLangKey',1]);
    }
...
function gettagcount($appid){
    ...
    Hook::listen('lang_parse',$groupdata,['getTaggroupLangKey',1]);
    $arr = [];
    foreach ($groupdata as $key => $val) {
        $arr[$key] = array('cid' => $key, 'text' => $val['catname'],'num'=>$val['num'],'langkey'=>isset($val['langkey']) ? $val['langkey']:'');
