...
if ($operation == 'addsearch') {//增加关键词搜索次数
    ...
    Hook::listen('lang_parse',$folderdatanum,['getFolderLangKey',1]);
...
elseif ($operation == 'searchmenu_num') {
    ...
    // if($lang) $wheresql .= " and (r.lang = '".$lang."' or r.lang = 'all' ) ";
    //关键词条件
    ...
    elseif($skey == 'sys'){
        ...
        $finish = (count($datas) >= $prepage) ? false:true;
        $tmpdata = [];
        foreach($datas as $v){
...
elseif($operation == 'getpagesetting'){
    ...
    Hook::listen('lang_parse',$vappdata,['getFolderLangData',1]);
...
elseif ($operation == 'getscreen') {//获取筛选项
    ...
    exit(json_encode(array('success' => true, 'data' => $pichomefilterfileds,'tagdata'=>$tagdata,'shape'=>$shapelable,'folderdata'=>$folderdata,'tabdata'=>$tabdata)));
...
elseif ($operation == 'createshare') {//分享
    ...
    if ($share = C::t('pichome_share')->add_share($rid)) {
        exit(json_encode(array('success' => $share['shareurl'])));
    } else {
        exit(json_encode(array('error' => true)));
    }
...
elseif($operation == 'realfianllypath'){
    $path = $_GET['path'] ? Pdecode($_GET['path'],'',0):'';
    ...
    if($ret['error']){
        exit(json_encode(['success'=>false,'msg'=>$ret['error']]));
    }else{
    ...
    foreach($taskarr as $k=>$v){
        if(intval($aitaskdata[$k])>0)   $datanums[] = ['lablename'=>$taskarr[$k],'data'=>$aitaskdata[$k] ? intval($aitaskdata[$k]):0];
    }
    $apppath = $appdata['path'];
    $writeable = is_writeable($apppath) ? true :false;
    if($appdata['type'] == 1 && (!$writeable || getglobal('config/allowDirectoryEditFilename'))) unset($datanums[0]);
    $queryInterval = 0;
    if(!$queryInterval){
    ...
    if($appdata['type'] == 1 || $appdata['type'] == 3){
        //计算待生成缩略图总文件数
       if($filenum = DB::result_first("select count(rid) as num from %t where appid = %s and isdelete = 0",array('pichome_resources',$appid))) {
           $getthumbnum = DB::result_first("select count(t.rid) from %t t left join %t r on r.rid=t.rid where
      r.appid = %s and r.isdelete = 0 and t.sstatus = %d ", ['thumb_record', 'pichome_resources', $appid, 1]);
           foreach (DB::fetch_all("select * from %t where donum < totalnum ", ['task_record']) as $v) {
               if ($v['idtype'] == 0) {
                   $folderdata = DB::fetch_first("select * from %t where fid = %s", [$this->_table, $v['fid']]);
                   Hook::listen('lang_parse', $folderdata, ['getFolderLangData']);
                   $datanums[] = ['lablename' => $folderdata['name'], 'data' => $v['donum'] . '/' . $v['totalnum']];
                   $queryInterval = 3;
               }
           }
    ...
    foreach($taskrecorddata as $v){
        if($v['idtype'] == 0){
            if($v['totalnum']<1) continue;
            $folderdata = C::t('pichome_folder')->fetch($v['idvalue']);
            Hook::listen('lang_parse',$folderdata,['getFolderLangData']);
            $datanums[] = ['lablename'=>lang('auto_stand').':'.$folderdata['fname'],'data'=>$v['donum'].'/'.$v['totalnum']];
            if($v['donum'] < $v['totalnum']) $queryInterval = 3;
        }
