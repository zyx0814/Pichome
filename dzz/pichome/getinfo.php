...
foreach(DB::fetch_all("select appid,path from %t where `type` = %d and isdelete < 1",array('pichome_vapp',1)) as $v){
...
//查询符合执行条件的数据
$datas = DB::fetch_all("select ra.rid,ra.getdonum,v.path from %t ra left join %t v on ra.appid=v.appid where ra.isget < 1 and ra.appid in(%n) and ra.getinfotime < v.dateline
