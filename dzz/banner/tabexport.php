...
$fileds= isset($_GET['fileds']) ? trim($_GET['fileds']):'';
//专辑数据
...
$notallow = array('viewperm','topping_at','is_hidden','is_recommed');
if($fileds){
    $hforms = [];
    ...
    $hforms = [];
    foreach($forms as $key => $val){
        if(!in_array($key,$notallow)){
            $hforms[$key] = $val;
        }
    }
}
$tids = isset($_GET['tid']) ? trim($_GET['tid']):'';
if(!$tids){
    ...
    if($perpage)$limitsql = "limit $start,$perpage";
    else $limitsql = '';
    $total = 0; //总条数
...
foreach(DB::fetch_all("select tid from %t where gid =%d and tid in(%n)",['tab',$gid,$tids]) as $v){
    ...
    foreach ($forms as $key => $val) {
        ...
        if ($key == 'cat') {
            ...
            $formdatas[$key]['value'] = $cdata[$key] ? dgmdate($cdata[$key],'Y-m-d H:i:s'):'';
...
// Create a first sheet
    ...
    $name = md5($tidsstr.$fileds);
    $content = ob_get_contents();
    $html = DZZ_ROOT.'data/attachment/cache/'.$name.'.html';
//取得php页面输出的全部内容
    $fp = fopen($html, "w+");
    fwrite($fp, $content);
    fclose($fp);
    ...
    if($exporttype == 'word') $exporttype = 'docx';
    $returnurl = $onlyoffice->convertHtmlToword($data,$exporttype);
    $filename = $_G['setting']['attachdir'].$returnurl;
    $filesize = filesize($filename);
    $chunk = 10 * 1024 * 1024;
    if (!$fp = @fopen($filename, 'rb')) {
        exit(lang('export_failure'));
    ...
    dheader('Last-Modified: ' . gmdate('D, d M Y H:i:s', TIMESTAMP) . ' GMT');
    dheader('Content-Encoding: none');
    dheader('Content-Disposition: attachment; filename=' . $gdata['name'].'.'.$exporttype);
    dheader('Content-Type: application/octet-stream');
    dheader('Content-Length: ' . $filesize);
    @ob_end_clean();
    if (getglobal('gzipcompress')) @ob_start('ob_gzhandler');
...
function getsearchlistbysearchfiled($searchfiled,$gdata,$limit=10)
{
    $fixedform = array(
        'topping_at' => array(
            'flag' => 'topping_at',
    ...
    foreach ($searchlist as $flag => $value) {
        ...
        if (!$value['status']) {
            ...
            } elseif ($form = C::t('form_setting')->fetch_by_flag($flag,$limit)) {
                $v = $form;
                $v['multiple'] = $value['multiple'];
                $v['range'] = $value['range'];
                $v['dw'] = $value['dw'];
