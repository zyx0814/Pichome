...
foreach(DB::fetch_all("select appid from %t where isdelete =2",array('pichome_vapp')) as $v){
    C::t('pichome_vapp')->delete_vapp_by_appid($v['appid']);
}
dzz_process::unlock($processname);
if(DB::result_first("select count(appid) from %t where isdelete =2 ",array('pichome_vapp'))){
