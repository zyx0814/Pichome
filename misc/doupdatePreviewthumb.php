...
for($i=0;$i<$processnum;$i++){
    $processname = 'DZZ_LOCK_PICHOMEGETTHUMBUPREVIEWPDATE'.$i;
...
$datas = DB::fetch_all("select r.rid,r.appid,t.*,ra.path as fpath,v.path,v.type,least(t.lchktimes,t.schktimes) as mintimes
...
order by r.dateline asc,mintimes asc limit $start,$limit",array('thumb_preview','pichome_resources','pichome_resources_attr','pichome_vapp',6,$appids));
if($datas){
    foreach($datas as $v){

        $processname1 = 'PICHOMEUPDATEPREVIEWTHUMB_'.$v['id'];
        ...
        //更新当前数据获取缩略图执行次数和时间
        if($v['schk']){
            C::t('thumb_preview')->update($v['id'],array('schktimes'=>intval($v['schktimes'])+1,'sdateline'=>TIMESTAMP));
            $thumbsign = 'small';
        }elseif(!$v['lstatus']){
            C::t('thumb_preview')->update($v['id'],array('lchktimes'=>intval($v['lchktimes'])+1,'ldateline'=>TIMESTAMP));
            $thumbsign = 'large';
        }else{
            dzz_process::unlock($processname1);
            continue;
...
left join %t ra on ra.rid=r.rid 
left join %t v on r.appid = v.appid 
where (t.schk = 1 or t.lchk = 1) and  ((t.lchktimes+t.schktimes) < %d) and v.isdelete < 1   and r.isdelete = 0 and r.appid in(%n)",array('thumb_preview','pichome_resources','pichome_resources_attr','pichome_vapp',6,$appids))){
        sleep(2);
