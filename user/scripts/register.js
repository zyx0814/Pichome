...
function addFormEvent(formid, focus){
	jQuery('#username').on('blur keyup', function () {
		if(this.value == '') {
			errormessage(jQuery(this), __lang.username_will);
		}else{
			errormessage(jQuery(this),'');
		}
	});
	jQuery('#nickname').on('blur keyup', function () {
		if(this.value == '') {
	...
	jQuery('#phone').on('blur keyup', function () {
		if(this.value == '') {
			errormessage(jQuery(this),__lang.sms_phone_empty);
		}else{
			checkphone(jQuery(this));
		}
	});
	jQuery('#sms_code').on('blur keyup', function () {
		if(this.value == '') {
			errormessage(jQuery(this),__lang.sms_code_empty);
		}else{
			errormessage(jQuery(this),'');
		}
	});
...
function checkphone(el) {
	...
	var isphone = /^1[3456789]\d{9,10}$/;  
	if(!isphone.test(phone)) {
		errormessage(el, __lang.user_phone_illegal);
		return;
	}
	jQuery.getJSON('user.php?mod=ajax&action=checkphone&phone=' + phone, function(json) {	
			errormessage(el, json.error||'');	
	});
}
