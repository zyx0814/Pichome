 ...
 */
	...
	if(type) {
		jQuery('#message_tip_alert').attr('class',' alert alert-'+type);
		if(type == 'info'){
		var spantype ='<span class="dzz dzz-info-outline spantype" ></span>'	;
			}else if(type == 'success'){
				var spantype ='<span class="dzz dzz-notification-success spantype" ></span>';	
			}else if(type == 'danger'){
				var spantype ='<span class="dzz dzz-clear spantype" ></span>';
			}else if(type == 'warning'){
				var spantype ='<span class="dzz dzz-error spantype"></span>';
			}
	}
	else  {
	...
	switch(position){
		case 'left-top':
			el.css({right:'auto',top:0,left:0,bottom:'auto'}).animate({left:0},delay);
			if(timeout){
		...
		case 'bottom-center'://bottom-center
			el.css({left:'50%','marginLeft':-width/2,bottom:-height,right:'auto',top:'auto'}).animate({'bottom':0},delay);;
			if(timeout){
				messageTimer=window.setTimeout(function(){
					el.animate({bottom:-height},delay,function(){
						el.remove();
					});
					if(typeof(callback)=='function') callback();
				},timeout);
