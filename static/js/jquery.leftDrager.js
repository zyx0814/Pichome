...
(function($)
{
	...
	$.fn.leftDrager_layout = function(resizefunc,options)
	{
		...
		var setPosition=function(xx,flag){
			...
			if(options.minWidth > 0 && xx <= options.minWidth){
				xx = options.minWidth;
			}
			if(flag===true){
				if(xx<10){
			...
			$mainContainer.css({"margin-left": left});
		...
		var dragerClick=function(flag,nocookie){
			if(flag==='hide'){
				...
				jQuery(document).trigger('leftDrager_hide');
			}else if(flag==='show'){
				...
				jQuery(document).trigger('leftDrager_show');
			}else{
		...
		var dragging=function(){
			$this.off('mousedown').on('mousedown',function(e){
				 ...
				 $(document).mousemove(function(e){
					  ...
					  setPosition(xx-ox);
					  jQuery(document).trigger('leftDrager_mousemove');
				 });
				 $(document).mouseup(function(e) {
					 ...
					 var xx=e.clientX;
					  ...
					  jQuery(document).trigger('leftDrager_mouseup');
				 });
			});
		};
