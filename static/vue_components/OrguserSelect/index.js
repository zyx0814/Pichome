...
const orguserselect = {
	...
	template: `
		<el-popover
			...
			:disabled="disabled">
			<el-scrollbar class="page-component__scroll" max-height="200px">
				<el-tree
					v-if="popovervisible"
					class="orguser-tree"
			...
			<template #reference>
				<div class="el-cascader" 
					...
					@mouseover="Inputbox.showclose=true"
					@mouseleave="Inputbox.showclose=false" 
					ref="slot-cascader" 
					@click="popovervisible=true"
					:class="{'is-disabled':disabled}">
					<div class="el-input el-input--suffix " :class="{'is-focus':popovervisible,'is-disabled':disabled}">
						<div class="el-input__wrapper">
							<input type="text" readonly="readonly" :placeholder="Inputbox.data.length?'':langTxt.text1" class="el-input__inner" :style="{height: Inputbox.height+'px'}">
					...
					<div class="el-cascader__tags" v-if="Inputbox.data.length" ref="cascader_tags" style="z-index:100;">
						<template v-for="item in Inputbox.data">
							<span class="el-tag el-tag--info el-tag--small el-tag--light">
								...
								<el-icon v-if="!disabled" class="el-tag__close" @click.stop="InputboxDelete(item)" style="z-index:100;">
									<Close></Close>
								</el-icon>
							</span>
						</template>
	...
	methods:{
		async handlefilterText(val){
			var res = await axios.post('index.php?mod=pichome&op=orgtree&do=search',{
				str:val
			});
			if(res == 'intercept'){
				return false;
		...
		async TreeLoadNode(node, resolve){
			...
			for (let index = 0; index < json.length; index++) {
				...
				if(item.type == 'default' && !this.isdefault){
					continue;
				}
				if(item.type == 'user'){
					item['leaf'] = true;
		...
		handleClickOutside(event) {
			const self = this;
			if (self.$refs['slot-cascader'].contains(event.target) || (self.$refs['popover_'+this.defaulttype] && self.$refs['popover_'+this.defaulttype].popperRef && self.$refs['popover_'+this.defaulttype].popperRef.contentRef.contains(event.target))) return;
			this.PopoverHide();
		},
		PopoverHide(val){
			const self = this;
		...
		handleEmitChange(){
			var self = this;
			self.$emit('change',this.Inputbox.data,this.defaulttype);
		},
		handleWindowResize(){
			var self = this;
